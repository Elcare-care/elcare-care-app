...
/// Token whitelist registry entry (Issue #208).
///
/// Replaces the flat Vec whitelist with a map-based registry that preserves
/// history and enables O(1) membership checks. Each token address maps to an
/// entry tracking when it was added, who added it, and whether it's currently
/// active. Removed tokens are soft-deleted (active: false) to maintain audit trail.
#[contracttype]
#[derive(Clone)]
pub struct TokenWhitelistEntry {
    /// Ledger timestamp when the token was added to the whitelist.
    pub added_at: u64,
    /// Address of the admin who added the token.
    pub added_by: Address,
    /// Whether the token is currently active (true) or removed (false).
    /// Removed entries are preserved for historical audit.
    pub active: bool,
}
