...
export function ListingForm({ listing, onSuccess, onCancel }: ListingFormProps) {
  ...
  /**
   * Parses the raw price input using bigint arithmetic (lib/amount.ts)
   * instead of a bare `parseFloat`. This rejects malformed input and
   * excess decimal precision (e.g. more places than the selected token
   * supports) up front rather than silently rounding at submission time.
   * The canonical, bigint-validated value is re-expressed as a JS number
   * for `form.price` so the existing contract-bound bound checks in
   * `validateListingForm` (MIN/MAX_PRICE_XLM) keep applying unchanged.
   */
  function handlePriceChange(raw: string, token: TokenConfig) {
    if (raw.trim() === "") {
      setPriceInputError(null);
      setForm((cur) => ({ ...cur, price: NaN }));
      return;
    }
    ...
    if (result.valid && result.baseUnits !== null) {
      setPriceInputError(null);
      setForm((cur) => ({ ...cur, price: Number(baseToDisplay(result.baseUnits!, token)) }));
    } else {
      setPriceInputError(result.message);
      setForm((cur) => ({ ...cur, price: NaN }));
    }
  }
