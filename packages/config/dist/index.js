...
const NetworkEnum = z.enum(['testnet', 'mainnet'], {
    error: 'STELLAR_NETWORK must be "testnet" or "mainnet". ' +
...
const NetworkPassphraseEnum = z.enum([
    ...
    error: 'STELLAR_NETWORK_PASSPHRASE must match the selected network. ' +
...
const ContractIdSchema = z.string().min(56).max(56).startsWith('C', {
    message: 'Contract ID must be exactly 56 characters and start with "C" (Stellar contract address format).',
});
...
const IndexerUrlSchema = z.string().url({
    message: 'INDEXER_URL must be a valid URL with http/https scheme (e.g., http://localhost:4000).',
});
const OperatorTokenSchema = z.string().min(32, {
    message: 'OPERATOR_TOKEN must be at least 32 characters for security. Generate with: openssl rand -hex 32',
});
const IndexerConfigSchema = z.object({
    ...
    operatorAllowlist: z
        ...
        ? val
            ...
            .filter(Boolean)
        : [])
        .refine((ips) => ips.length === 0 || ips.every(isValidIp), {
        message: 'OPERATOR_ALLOWLIST must be comma-separated valid IP addresses (IPv4 or IPv6).',
    }),
});
...
function validateNetworkConsistency(config) {
    ...
    if (config.networkPassphrase !== expected) {
        throw new ValidationError(`[NETWORK] Network passphrase mismatch. ` +
...
function validateContractConsistency(config) {
    ...
    if (!config.marketplaceContractId || config.marketplaceContractId.length !== 56) {
        throw new ValidationError(`[CONTRACT] MARKETPLACE_CONTRACT_ID must be exactly 56 characters. ` +
    ...
    if (config.launchpadContractId) {
        if (config.launchpadContractId.length !== 56) {
            throw new ValidationError(`[CONTRACT] LAUNCHPAD_CONTRACT_ID must be exactly 56 characters. ` +
...
function validateIndexerUrlConsistency(config) {
    ...
    if (url.hostname === 'localhost' || url.hostname === '127.0.0.1') {
        console.warn(`[CONFIG] INDEXER_URL points to localhost. ` +
...
export function loadConfig() {
    ...
    if (!networkResult.success) {
        throw new ValidationError(`[NETWORK] Invalid network configuration:\n` +
    ...
    if (!contractResult.success) {
        throw new ValidationError(`[CONTRACT] Invalid contract configuration:\n` +
    ...
    if (!indexerResult.success) {
        throw new ValidationError(`[INDEXER] Invalid indexer configuration:\n` +
    ...
    if (!databaseResult.success) {
        throw new ValidationError(`[DATABASE] Invalid database configuration:\n` +
    ...
    if (!ipfsResult.success) {
        throw new ValidationError(`[IPFS] Invalid IPFS configuration:\n` +
    ...
    if (!keeperResult.success) {
        throw new ValidationError(`[KEEPER] Invalid keeper configuration:\n` +
    ...
    if (!versionResult.success) {
        throw new ValidationError(`[VERSION] Invalid version configuration:\n` +
